...
author: "Tara Eicher, Kyle D. Spencer, Jalal K. Siddiqui, Raghu Machiraju,
Ewy A. Mathé"
date: "2/24/2023"
...
can be found on our publications "IntLIM: integration using linear models of 
metabolomics and gene expression data" and "IntLIM 2.0: Identifying Multi-Omic 
Relationships Dependent on Discrete or Continuous Phenotypic Measurements".
The workflow involves the following steps:
1) Input multi-omic data files.
2) Filter data sets by analyte abundance and imputed values.
3) Run the linear model to extract phenotype-dependent analyte associations.
4) Filter results by interacton coefficients, p-values, interaction coefficient percentiles, and r-squared values.
5) Perform cross-validation and/or null model validation.
6) Plot/visualize specific analyte associations.
...
data corresponding to 1-2 analyte types, optional metadata for each of these types,
...
analyteType1MetaData  | rows are analytes, analyte features are columns
analyteType2MetaData  | rows are analytes, analyte features are columns
sampleMetaData        | rows are samples, sample features are columns

For the analyte data files, the first row contains the sample IDs and the first
column contains the analyte IDs.
...
column that contains the name of the analyte or sample (sample id,
name, etc).
...
and metabolite abundance and metabolites with more than 80% imputed values.  The
__cov.cutoff__ parameter can also be used to filter by coefficient of variation,
which is not shown in this example.
We henceforth have 1303 genes and 231 metabolites for 20 cell line samples.
...
containing slots for un-adjusted and False Discovery Rate (FDR)-adjusted p-values
for the “(g:p)” interaction coefficient.  A significant FDR-adjusted p-value implies
that the gene-metabolite association is dependent on the phenotype.
...
This is done by setting the __coefficient__ argument to "stype" (for phenotype) or "analyte"
(for independent analyte level).
...
For discrete phenotypes, __PlotPair()__ allows the user to plot a chosen analyte
...
Eicher T, Spencer KD, Siddiqui JK, Machiraju R, Mathé EA. 
IntLIM 2.0: Identifying Multi-Omic Relationships Dependent on Discrete or 
Continuous Phenotypic Measurements. 2023 Feb;01:vbad009.
