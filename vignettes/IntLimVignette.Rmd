...
---
title: "IntLIM:  Integration through Linear Modeling (Discrete)"
...
vignette: >
  %\VignetteIndexEntry{IntLIM:  Integration through Linear Modeling (Discrete)}
...
r-squared values.
```{r}
IntLIM::DistRSquared(IntLimResults = myres)
```
...
```{r}
IntLIM::PValueBoxPlots(IntLimResults = myres)
```
...
p-values (default set at 0.05) between the two groups.  The output 
is a list of significant analyte pairs.

```{r}
myres.sig <- IntLIM::ProcessResults(inputResults = myres,
                                       inputData = inputDatafilt,
                                       pvalcutoff = 0.10, rsquaredCutoff = 0.2,
                                       interactionCoeffPercentile = 0.5)
```

The __PlotPair()__ function  allows the user to plot a chosen analyte 
...
IntLIM::OutputResults(inputResults=myres.sig,filename=paste(result_dir, "MyResults.csv", sep = "\\"))
```
...
Note that __PlotFoldOverlapUpSet__ is currently commented out to avoid large
packages sizes when building vignettes. Please uncomment while running this vignette.
```{r}
...
the pair is also significant.
Note that __PermutationCountSummary__ and __PermutationPairSummary__ are currently 
commented out to avoid large
packages sizes when building vignettes. Please uncomment while running this vignette.
```{r}
perm.res <- IntLIM::PermuteIntLIM(data = inputDatafilt, stype = "PBO_vs_Leukemia", outcome = 1,
                   ...
                   num.permutations = 5)
#countSummary <- IntLIM::PermutationCountSummary(permResults = perm.res, inputResults = myres.sig,
#                              plot = TRUE)
#pairSummary <- IntLIM::PermutationPairSummary(permResults = perm.res, inputResults = myres.sig,
#                              plot = TRUE)
```
