...
command allows the user to select a column from the sample meta data that 
color-codes the samples into two categories (two cancer types, tumor vs. 
non-tumor, etc).  For the sample set, we select PBO (prostate/breast/ovarian 
cancers) vs. the leukemia group.  
...
allows the user to select a column from the sample meta data for the two categories 
to be compared (two cancer types, tumor vs. non-tumor, etc).  Currently, IntLIM 
supports continuous data and comparison of two categories (__"PBO_vs_Leukemia"__ 
is selected). The resulting object from the analysis is an IntLimResults object 
...
that the slope of gene-metabolite association in one phenotype is different from 
the other. The __RunIntLim()__ function is based heavily on the MultiLinearModel 
...
```
The __MarginalEffectsGraph()__ function allows the user to observe the marginal 
effects of phenotype for a given pair of analytes.
The __MarginalEffectsGraphDataframe()__ function creates the input for 
__MarginalEffectsGraph()__, given the IntLIM results, the input data, and the 
analytes of interest.
```{r}
IntLIM::MarginalEffectsGraph(dataframe = 
      IntLIM::MarginalEffectsGraphDataframe(inputResults = myres.gm, 
      ...
      title = "Marginal Effects - DLG4 and (p-Hydroxyphenyl)lactic acid")
```

The __pvalCorrVolcano()__ function allows users to observe a volcano plot comparing 
the Spearman correlation difference between groups to the –log10(FDR-adjusted p-value).
...
Process the results and filter pairs of analytes based on adjusted p-values and 
differences in correlation coefficients between groups 1 and 2.  Then plot heatmap 
of significant analyte pairs
...
__NULL__ for continuous phenotypes.
...
The __CorrHeatmap()__ produces a clustered heatmap of the analyte Spearman 
correlations for each of these groups.  
...
A __PlotGMPair()__ function  allows the user to plot a chosen analyte 
...
```{r}
IntLIM::PlotPair(inputDatafilt,stype="PBO_vs_Leukemia", outcome = 1, 
                 independentVariable = 2, 
                 outcomeAnalyteOfInterest = "(p-Hydroxyphenyl)lactic acid", 
                 independentAnalyteOfInterest = "DLG4")
```
...
```{r eval = FALSE}
feats <- list(panelnbr = "factor", cellnbr = "numeric",
              PBO_vs_Leukemia = "factor", drugscore = "numeric")
crossValResults <- RunCrossValidation(inputFile = csvfile,analyteType1id='id',analyteType2id='id',
                   class.feat = feats, analyteType1perc = 0.10, 
                   ...
                   folds = 4)
IntLIM::PlotFoldOverlapUpSet(crossValResults$processed[[1]])
```
##References
